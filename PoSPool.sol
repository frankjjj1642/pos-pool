...
interface Staking {
    ...
    // ------------------------------------------------------------------------
    function getLockedStakingBalance(
        address user,
        uint256 blockNumber
    ) external view returns (uint256);
...
abstract contract PoolContext {
    Staking private constant STAKING =
        Staking(0x0888000000000000000000000000000000000002);
    PoSRegister private constant POS_REGISTER =
    ...
    function _posAddressToIdentifier(
        address _addr
...
library PoolAPY {
    ...
    function clearOutdatedNode(
        ApyQueue storage queue,
        uint256 outdatedBlock
    ) internal {
    ...
    function enqueueAndClearOutdated(
        ApyQueue storage queue,
        ApyNode memory item,
        uint256 outdatedBlock
    ) internal {
...
contract PoSPool is PoolContext, Ownable, Initializable {
    ...
    uint256 public poolUserShareRatio = 0;
    ...
    ParamsControl public paramsControl =
    ...
    modifier onlyVotingEscrow() {
        require(
            msg.sender == votingEscrow,
            "Only votingEscrow can call this function"
        );
        _;
    }
    ...
    function register(
        ...
        require(
            ...
            "msg.value should be 1000 CFX"
        );
        _stakingDeposit(msg.value);
        _posRegisterRegister(
            indentifier,
            votePower,
            blsPubKey,
            vrfPubKey,
    ...
    function userSummary(
        address _user
    ) public view returns (UserSummary memory) {
    ...
    function poolAPY() public view returns (uint256) {
        ...
        for (uint256 i = apyNodes.start; i < apyNodes.end; i++) {
            ...
            totalWorkload = totalWorkload.add(
                node.available.mul(CFX_VALUE_OF_ONE_VOTE).mul(
                    node.endBlock - node.startBlock
                )
            );
        }
        ...
        if (_blockNumber() > lastPoolShot.blockNumber) {
            ...
            totalWorkload = totalWorkload.add(
                lastPoolShot.available.mul(CFX_VALUE_OF_ONE_VOTE).mul(
                    _blockNumber() - lastPoolShot.blockNumber
                )
            );
        }

        return
            totalReward.mul(RATIO_BASE).mul(ONE_YEAR_BLOCK_COUNT).div(
                totalWorkload
            );
    }
    ...
    function addToFeeFreeWhiteList(
        address _freeAddress
    ) public onlyOwner returns (bool) {
        return feeFreeWhiteList.add(_freeAddress);
    }

    function removeFromFeeFreeWhiteList(
        address _freeAddress
    ) public onlyOwner returns (bool) {
    ...
    function _withdrawPoolProfit(uint256 amount) public onlyOwner {
        ...
        amount = _selfBalance();
        _poolSummary.interest = 0;
        _poolSummary.totalInterest = _poolSummary.totalInterest.add(
            _selfBalance()
        );
        address payable receiver = payable(msg.sender);
        receiver.transfer(amount);
    ...
    function _retireUserStake(
        ...
        userOutqueues[_addr].enqueue(
            VotePowerQueue.QueueNode(votePower, endBlockNumber)
        );
    ...
    function _restakeUserStake(address _addr) public onlyOwner {
        userSummaries[_addr].unlocked += userOutqueues[_addr]
            .collectEndedVotes();
